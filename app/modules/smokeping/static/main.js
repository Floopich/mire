...
function loadSmokepingGraphs() {
    ...
    fetch(mireUrl('/api/smokeping/targets'))
        .then(function(r) { return r.json(); })
        .then(function(targets) {
            if (!targets || targets.length === 0) {
                noData.textContent = T.smokeping_no_data || T['mire.smokeping.smokeping_no_data'] || 'Could not load Smokeping graphs.';
            ...
            targets.forEach(function(target) {
                ...
                img.alt = target;
                img.src = mireUrl('/api/smokeping/graph/' + encodeURIComponent(target) + '/' + _smokepingSpan);
                img.onerror = function() {
                    var fallback = document.createElement('div');
                    fallback.className = 'no-data-msg';
                    fallback.style.display = 'block';
                    fallback.textContent = T.smokeping_no_data || T['mire.smokeping.smokeping_no_data'] || 'Could not load graph.';
...
function openSmokepingSetupModal() {
    window.MireModal.open('smokeping-setup-modal');
}
function closeSmokepingSetupModal() {
    window.MireModal.close('smokeping-setup-modal');
}
