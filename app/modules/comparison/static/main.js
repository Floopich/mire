...
function _cmpRunComparison() {
    ...
    var url = mireUrl('/api/comparison?from_a=' + encodeURIComponent(_cmpToISO(fromA)) +
    ...
    fetch(url)
        ...
        .then(function(data) {
            ...
            if (data.error) {
                ...
                _cmpLastResult = null;
                window.__mireComparisonResult = null;
                return;
            }
            _cmpLastResult = data;
            window.__mireComparisonResult = data;
...
function _cmpBuildTimeLabels(hourLabels, period) {
    ...
    return hourLabels.map(function(h) {
        return mireFormatXAxisLabel(periodStartMs + (h * 3600000), String(rangeSeconds) + 's');
    });
}
...
function _cmpRenderCharts(data) {
    ...
    var labelA = T['mire.comparison.period_a'] || T.period_a || 'Period A';
    var labelB = T['mire.comparison.period_b'] || T.period_b || 'Period B';
    ...
    if (pa.timeseries.length === 0 && pb.timeseries.length === 0) {
        var placeholder = document.getElementById('comparison-placeholder');
        placeholder.style.display = 'block';
        placeholder.textContent = T['mire.comparison.no_data_period'] || 'No data in selected period';
        return;
    }
...
function _cmpRenderDeltaTable(data) {
    ...
    _cmpAppendDeltaRow(tbody, T['mire.comparison.ds_power'] || 'DS Power',
        pa.avg.ds_power, pb.avg.ds_power, 'dBmV', delta.ds_power, false, false);
    _cmpAppendDeltaRow(tbody, T['mire.comparison.ds_snr'] || 'DS SNR',
        pa.avg.ds_snr, pb.avg.ds_snr, 'dB', delta.ds_snr, true, false);
    _cmpAppendDeltaRow(tbody, T['mire.comparison.us_power'] || 'US Power',
    ...
    if (_cmpPeriodSupportsDocsisErrors(pa) || _cmpPeriodSupportsDocsisErrors(pb)) {
        _cmpAppendDeltaRow(tbody, T['mire.comparison.uncorr_errors'] || 'Uncorr. Errors',
    ...
    var verdictLabel = T['mire.comparison.' + delta.verdict] || delta.verdict;
    var verdictClass = delta.verdict === 'improved' ? 'cmp-good' :
                       delta.verdict === 'degraded' ? 'cmp-bad' : '';
    var tr = document.createElement('tr');
    _cmpAddCell(tr, T['mire.comparison.health'] || 'Health');
