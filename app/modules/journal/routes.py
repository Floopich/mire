...
def api_journal_export():
    ...
    # Markdown
    lines = ["# Mire Incident Journal Export", ""]
...
def api_incident_report(incident_id):
    ...
    pdf_bytes = generate_incident_report(
        incident, entries, snapshots, speedtests,
    ...
    response.headers["Content-Disposition"] = f'attachment; filename="Mire_Plainte_{safe_name}_{ts}.pdf"'
    return response
