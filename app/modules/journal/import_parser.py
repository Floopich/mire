...
# Contexte belge : les fichiers importes arrivent en fr ou nl aussi bien
# qu'en de. Aucune cle commune aux trois tables n'a de valeur divergente.
MONTH_NAMES = {**MONTH_NAMES_DE, **MONTH_NAMES_FR, **MONTH_NAMES_NL}

MONTH_HEADER_RE = re.compile(
    r"^(" + "|".join(sorted(MONTH_NAMES, key=len, reverse=True)) +
    r")\s*\(?(\d{4})?\)?\s*$",
    re.IGNORECASE,
)
