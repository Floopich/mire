...
class MQTTPublisher:
    def __init__(self, host, port=1883, user=None, password=None,
        ...
        self.public_url = public_url.rstrip("/") if public_url else f"http://mire:{web_port}"
    ...
    def publish_discovery(self, device_info=None):
        ...
        for key, name, unit, icon, enabled in summary_sensors:
            topic = f"{self.ha_prefix}/sensor/mire/{key}/config"
            config = {
                "name": name,
                "unique_id": f"mire_{key}",
        ...
        health_topic = f"{self.ha_prefix}/binary_sensor/mire/health/config"
        health_config = {
            "name": "DOCSIS Health",
            "unique_id": "mire_health",
        ...
        # --- Mire Status as binary_sensor with device_class=running ---
        status_topic = f"{self.ha_prefix}/binary_sensor/mire/status/config"
        status_config = {
            "name": "Status",
            "unique_id": "mire_status",
    ...
    def publish_channel_discovery(self, ds_channels, us_channels, device_info=None):
        ...
        for ch in ds_channels:
            ...
            topic = f"{self.ha_prefix}/sensor/mire/{obj_id}/config"
            config = {
                "name": f"DS Channel {ch_id}",
                "unique_id": f"mire_{obj_id}",
            ...
            snr_topic = f"{self.ha_prefix}/sensor/mire/{snr_obj_id}/config"
            snr_config = {
                "name": f"DS Channel {ch_id} SNR",
                "unique_id": f"mire_{snr_obj_id}",
