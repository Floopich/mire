...
function lowQamLegendHintForContext(pg) {
    if (isUsDocsis31Upstream(pg)) {
        return T['mire.modulation.low_qam_legend_hint_d31_us'] ||
            T['low_qam_legend_hint_d31_us'] ||
            'US DOCSIS 3.1: 64QAM and below count as Low-QAM; 128QAM is borderline.';
    }
    if (isUsDocsis30Upstream(pg)) {
        return T['mire.modulation.low_qam_legend_hint_d30_us'] ||
...
function fetchOverview() {
    fetch(mireUrl('/api/modulation/distribution?direction=' + _modDirection + '&days=' + _modDays))
        ...
        .then(function(data) {
            ...
            var noData = document.getElementById('modulation-no-data');
            if (noData) {
                noData.textContent = T['mire.modulation.no_data'] || T.no_data || 'No modulation data available.';
...
function updateOverviewKPIs(data) {
    ...
    if (lqHint) {
        lqHint.textContent = T['mire.modulation.low_qam_denominator_hint'] ||
    ...
    if (dHint) {
        dHint.textContent = data.sample_count + ' / ' + data.expected_samples +
            ' ' + (T['mire.modulation.samples'] || T.samples || 'samples');
    }
}
...
function rangeLabelForCapacity(data) {
    if (data && data.date) {
        return (T['mire.modulation.capacity_selected_day'] || 'Selected day') + ': ' + data.date;
    }
    if (_modDays === 1) {
        return T['mire.modulation.capacity_today_period'] || 'Today';
    }
    var template = T['mire.modulation.capacity_selected_days'] || 'Selected period: {days}d';
...
function capacityFamilyLabel(family) {
    ...
    return T['mire.modulation.capacity_family_' + normalized] || fallback;
}
...
function capacityCoverageText(summary) {
    ...
    var template = T['mire.modulation.capacity_coverage_detail'] ||
...
function capacityPartialCaveatText(summary) {
    ...
    if (families.length > 0) {
        var template = T['mire.modulation.capacity_partial_caveat'] ||
    ...
    return T['mire.modulation.capacity_partial_caveat_generic'] ||
...
function capacityStatusText(summary) {
    if (!summary) return '\u2014';
    var key = 'mire.modulation.capacity_status_' + summary.status;
    var fallback = {
    ...
    if (summary.unsupported_channel_samples > 0) {
        text = (T['mire.modulation.capacity_partial_coverage'] || 'Partial estimate') + ' · ' + text;
    }
    return text;
}
...
function capacityTariffText(summary) {
    if (!summary || !summary.tariff_mbps) {
        return T['mire.modulation.capacity_no_tariff'] || 'No tariff configured';
    }
...
function updateCapacityHistory(data, rangeLabel) {
    ...
    if (label) {
        label.textContent = rangeLabel || (T['mire.modulation.capacity_selected_period'] || 'Selected period');
    }
...
function renderProtocolGroups(data) {
    ...
    groups.forEach(function(pg, idx) {
        ...
        var dirLabel = _modDirection === 'us'
            ? (T['mire.modulation.upstream'] || 'Upstream')
            : (T['mire.modulation.downstream'] || 'Downstream');
        ...
        var chLabel = pg.channel_count === 1
            ? (T['mire.modulation.channel_label'] || 'Channel')
            : (T['mire.modulation.channels_label'] || 'Channels');
        ...
        if (pg.degraded_channel_count > 0) {
            var degBadge = _el('span', 'mod-protocol-badge degraded',
                pg.degraded_channel_count + ' ' + (T['mire.modulation.degraded_channels'] || 'degraded'));
        ...
        kpiRow.appendChild(_buildMiniKPI(
            T['mire.modulation.health_index'] || 'Health Index',
            pg.health_index !== null ? pg.health_index.toFixed(1) : '\u2014',
            healthClass(pg.health_index)));
        kpiRow.appendChild(_buildMiniKPI(
            T['mire.modulation.low_qam_pct'] || 'Low-QAM %',
            pg.low_qam_pct.toFixed(1) + '%',
            lowQamClass(pg.low_qam_pct)));
        kpiRow.appendChild(_buildMiniKPI(
            T['mire.modulation.dominant_modulation'] || 'Dominant',
            pg.dominant_modulation || '\u2014', ''));
        kpiRow.appendChild(_buildMiniKPI(
            T['mire.modulation.degraded_channels'] || 'Degraded',
        ...
        barContent.appendChild(_el('div', 'chart-label',
            T['mire.modulation.distribution_chart'] || 'Modulation Distribution'));
        ...
        trendContent.appendChild(_el('div', 'chart-label',
            T['mire.modulation.trend_chart'] || 'Health Trend'));
...
function attachModulationDayClick(chart, container, days) {
    ...
    container.setAttribute(
        'title',
        T['mire.modulation.click_day_hint'] || 'Click a day bar to view per-channel details'
    );
...
function renderGroupTrendChart(pg, idx) {
    ...
    var chart = new uPlot({
        ...
        series: [
            ...
            {
                label: T['mire.modulation.health_index'] || 'Health Index',
                stroke: '#22c55e',
            ...
            {
                label: T['mire.modulation.low_qam_pct'] || 'Low-QAM %',
                stroke: '#ef4444',
...
function renderIntraday(data) {
    ...
    if (titleEl) {
        titleEl.textContent = (T['mire.modulation.intraday_title'] || 'Channel Detail') +
            ' \u2014 ' + data.date;
    }
    ...
    if (groups.length === 0) {
        container.appendChild(_el('div', 'no-data-msg',
            T['mire.modulation.no_data'] || 'No data for this day.'));
        return;
    }
        ...
        channels.forEach(function(ch) {
            ...
            if (ch.degraded_events && ch.degraded_events.length) {
                ...
                card.appendChild(_el('div', 'mod-channel-summary-text',
                    T['mire.modulation.no_degradation'] || 'No degradation observed'));
            }
...
function buildIntradayInsights(ch) {
    ...
    wrap.appendChild(_el(
        ...
        ch.degraded_events.length + ' ' + (T['mire.modulation.degraded_windows'] || 'degraded windows')
    ));
    wrap.appendChild(_el(
        'span',
        'mod-channel-insight',
        (T['mire.modulation.affected_samples'] || 'Affected') + ' ' + (ch.degraded_sample_pct || 0) + '%'
    ));
    if (ch.worst_modulation) {
        wrap.appendChild(_el(
            'span',
            'mod-channel-insight critical',
            (T['mire.modulation.worst_modulation'] || 'Worst') + ' ' + ch.worst_modulation
        ));
    }
    return wrap;
}
...
function buildIntradayEventList(events) {
    ...
    if (hiddenRows.length > 0) {
        ...
        function syncToggleLabel() {
            if (expanded) {
                toggle.textContent = T['mire.modulation.show_fewer_windows'] || 'Show fewer';
                toggle.classList.add('open');
            } else {
                toggle.textContent = '+' + hiddenRows.length + ' ' +
                    (T['mire.modulation.more_windows'] || 'more windows');
...
function buildIntradayEvent(evt) {
    ...
    impact.appendChild(_el('span', 'mod-event-share-label',
        T['mire.modulation.of_samples'] || 'of samples'));
...
function formatIntradayEventDuration(evt) {
    if (evt.point_in_time) {
        ...
        return T['mire.modulation.brief_dip'] || 'brief dip';
    }
...
function sampleUnit(count) {
    return count === 1
        ? (T['mire.modulation.sample_singular'] || 'sample')
        : (T['mire.modulation.sample_plural'] || 'samples');
}
...
function showDisclaimer(text) {
    ...
    if (el && textEl) {
        textEl.textContent = T['mire.modulation.disclaimer'] || text || '';
