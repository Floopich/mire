...
(function() {
    ...
    window.cmExportCsv = function(targetId) {
        ...
        triggerExport(mireUrl('/api/connection-monitor/export/' + targetId + '?start=' + range.start + '&end=' + range.end + '&resolution=' + lastResolution));
    };

    window.cmExportRawLog = function(targetId) {
        var range = getExportWindow();
        triggerExport(mireUrl('/api/connection-monitor/export/' + targetId + '?start=' + range.start + '&end=' + range.end + '&resolution=raw&format=pinglog'));
    };
    ...
    function pinCurrentDay() {
        ...
        fetch(mireUrl('/api/connection-monitor/pinned-days'), {
            method: 'POST',
    ...
    function renderPinnedDays(days) {
        ...
        days.forEach(function(day) {
            ...
            removeBtn.onclick = function(e) {
                ...
                fetch(mireUrl('/api/connection-monitor/pinned-days/' + day.date), { method: 'DELETE' })
                    .then(function() {
                        if (removedActiveDay) {
                            pinnedDayView = null;
                            document.querySelectorAll('[data-cm-range]').forEach(function(b) {
    ...
    function init() {
        ...
        fetch(mireUrl('/api/connection-monitor/capability'))
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    function loadTargets() {
        fetch(mireUrl('/api/connection-monitor/targets'))
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    function loadData() {
        ...
        var samplePromises = targets.map(function(t) {
            var url = mireUrl('/api/connection-monitor/samples/' + t.id + '?start=' + start + '&end=' + end + '&limit=0');
            if (pinnedDayView) {
                url += '&resolution=raw';
            } else if (maxPoints > 0) {
        ...
        var statsPromise = fetch(mireUrl('/api/connection-monitor/stats?start=' + start + '&end=' + end))
            .then(function(r) { return r.json(); });

        // Fetch outages for ALL targets in parallel
        var outagePromises = targets.map(function(t) {
            return fetch(mireUrl('/api/connection-monitor/outages/' + t.id + '?start=' + start + '&end=' + end))
                .then(function(r) { return r.json(); })
    ...
    function loadTraceHistory() {
        ...
        fetch(mireUrl('/api/connection-monitor/traces/' + trSelectedTargetId))
            .then(function(r) { return r.json(); })
    ...
    function toggleTraceDetail(row, traceId, toggleButton, detailId) {
        ...
        // Fetch detail
        fetch(mireUrl('/api/connection-monitor/trace/' + traceId))
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    window.cmRunTraceroute = function() {
        ...
        fetch(mireUrl('/api/connection-monitor/traceroute/' + trSelectedTargetId), { method: 'POST' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                btn.disabled = false;
