...
def _conn_info_with_booked(conn_info, config_manager):
    """Complete les infos de connexion avec les debits souscrits.

    Le modem ne les expose pas en mode bridge. booked_download / booked_upload
    sont en Mbit/s dans la configuration, la ligne tarifaire du rapport attend
    des kbit/s.
    """
    if conn_info.get("max_downstream_kbps") or not config_manager:
        return conn_info
    down = config_manager.get("booked_download", 0) or 0
    up = config_manager.get("booked_upload", 0) or 0
    if not down or not up:
        return conn_info
    merged = dict(conn_info)
    merged["max_downstream_kbps"] = int(down) * 1000
    merged["max_upstream_kbps"] = int(up) * 1000
    return merged
...
def api_report():
    ...
    conn_info = _conn_info_with_booked(get_state().get("connection_info") or {}, _config_manager)
    ...
    response.headers["Content-Disposition"] = f'attachment; filename="mire_incident_report_{ts}.pdf"'
    return response
