"""Incident Report PDF generator for Mire."""
...
def _default_warn_thresholds(ds_snr_warn_min=None):
    ...
    ds_w = ds.get("warning", [-10.0, 10.0])   # repli aligne sur mire.thresholds_voo
    us_w = us.get("warning", [27.0, 51.0])    # repli aligne sur mire.thresholds_voo
    return {
        "ds_power": f"{ds_w[0]} to {ds_w[1]} dBmV",
        "us_power": f"{us_w[0]} to {us_w[1]} dBmV",
        "snr": f">= {ds_snr_warn_min if ds_snr_warn_min is not None else snr.get('warning_min', 33.0)} dB",
    }
...
class IncidentReport(FPDF):
    """Custom PDF class for Mire incident reports."""
...
def generate_incident_report(incident, entries, snapshots, speedtests,
...
def generate_complaint_text(snapshots, config=None, connection_info=None, lang="en",
                            customer_name="", customer_number="", customer_address="",
                            current_analysis=None, comparison_data=None,
