"""Backup and restore for Mire data.
...
def _write_backup_archive(data_dir, archive_target, work_dir=None):
    ...
    with tempfile.TemporaryDirectory(
        prefix=".mire-backup-work-", dir=work_dir
    ) as tmp:
...
def create_backup_to_file(data_dir, dest_dir):
    ...
    filename = f"mire_backup_{ts}.tar.gz"
    ...
    try:
        with tarfile.open(fileobj=archive_bytes, mode="r:gz") as tar:
            ...
            if BACKUP_META_FILE not in names:
                raise ValueError("Missing backup_meta.json - not a valid Mire backup")
        ...
        with tarfile.open(fileobj=archive_bytes, mode="r:gz") as tar:
            for member in tar.getmembers():
                ...
                fd, staged_path = tempfile.mkstemp(
                    prefix=f".mire-restore-{member.name}.",
                    suffix=".tmp",
                    dir=data_dir,
                )
                try:
...
def list_backups(backup_dir):
    ...
    for fname in os.listdir(backup_dir):
        if not fname.startswith("mire_backup_") or not fname.endswith(".tar.gz"):
            continue
