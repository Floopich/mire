...
function _evidenceStatusLabel(status) {
    ...
    return _evidenceT('mire.evidence.status.' + safeStatus, safeStatus.replace('_', ' '));
}
...
function _evidenceActionLabel(item) {
    ...
    return _evidenceT('mire.evidence.action.' + (action || view || 'review'), 'Open related view');
}
...
function _evidenceSourceLabel(source) {
    return _evidenceT('mire.evidence.source.' + source.key, String(source.key || '').replace(/_/g, ' '));
}
...
function _evidenceBuildUrl() {
    ...
    if (incidentId) {
        return mireUrl('/api/evidence/checklist?incident_id=' + encodeURIComponent(incidentId));
    }
    ...
    return mireUrl('/api/evidence/checklist?from=' + encodeURIComponent(from) + '&to=' + encodeURIComponent(to));
}
...
function _evidenceSupportSummary(payload) {
    if (!payload) return '';
    var lines = [];
    lines.push(_evidenceT('mire.evidence.copy_heading', 'Mire evidence summary'));
    lines.push((_evidenceT('mire.evidence.copy_window', 'Window') + ': ' + payload.window.label + ' (' + payload.window.from + ' – ' + payload.window.to + ')'));
    (payload.items || []).forEach(function(item) {
        lines.push('- ' + _evidenceT(item.label_key, item.key) + ': ' + _evidenceStatusLabel(item.status) + (item.count ? ' (' + item.count + ')' : ''));
    });
    lines.push(_evidenceT('mire.evidence.copy_review_note', 'Review the details before sharing; this summary lists available evidence only.'));
    return lines.join('\n');
}
...
function _evidenceSetCopyState(state) {
    ...
    copy.textContent = state === 'success'
        ? _evidenceT('mire.evidence.copy_success', 'Copied')
        : state === 'failed'
            ? _evidenceT('mire.evidence.copy_failed', 'Copy failed')
            : defaultLabel;
    if (state) {
...
function _evidenceLoad() {
    ...
    if (!url) {
        ...
        placeholder.textContent = _evidenceT('mire.evidence.choose_window', 'Choose an incident or complete time range first.');
        return;
    }
