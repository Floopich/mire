...
def _events_csv_response(events, truncated=False):
    ...
    return Response(
        ...
        mimetype="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=mire-events-{stamp}.csv",
            "X-Mire-Export-Limit": str(_EVENTS_EXPORT_LIMIT),
            "X-Mire-Export-Truncated": "true" if truncated else "false",
        },
    )
