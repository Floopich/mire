...
# Le catalogue amont vise le marche allemand. On ecarte ce qui n a pas d objet
# pour Mire, et on garde en reserve ce qui pourrait servir plus tard.
REGISTRY_EXCLUDED_IDS = frozenset({
    # Profil de seuils allemand (vodafonekabelforum.de) : Mire embarque le sien.
    "community.thresholds_vfkd_community",
})

REGISTRY_HIDDEN_IDS = frozenset({
    # Cible les UDM Pro/SE ; compatibilite UCG non verifiee. Visible en activant
    # show_reserved_modules dans la configuration.
    "community.udm_wan_monitor",
})


def _filter_registry_modules(modules, config_mgr):
    """Ecarte les modules sans objet et masque ceux tenus en reserve."""
    show_reserved = bool(config_mgr and config_mgr.get("show_reserved_modules"))
    result = []
    for mod in modules:
        mod_id = mod.get("id", "")
        if mod_id in REGISTRY_EXCLUDED_IDS:
            continue
        if mod_id in REGISTRY_HIDDEN_IDS and not show_reserved:
            continue
        result.append(mod)
    return result
...
def api_modules_registry():
    ...
    modules = _filter_registry_modules(modules, config_mgr)
