...
class AppriseChannel(NotificationChannel):
    ...
    def _format_payload(cls, payload: NotificationPayload) -> dict[str, object]:
        ...
        apprise_payload: dict[str, object] = {
            "title": f"Mire: {event_type.replace('_', ' ').title()}",
...
class WebPushChannel(NotificationChannel):
    ...
    def _format_payload(cls, payload: NotificationPayload) -> dict[str, object]:
        ...
        title = f"Mire {severity}: {event_type.replace('_', ' ').title()}"
        body = str(payload.get("message", "") or "Mire notification")
        return {
            "title": title[:100],
            "body": body[:240],
            "severity": severity,
...
class DiscordWebhookChannel(NotificationChannel):
    ...
    def _format_embed(payload: NotificationPayload) -> dict[str, object]:
        """Convert a Mire notification payload into a Discord embed."""
...
class NotificationDispatcher:
    ...
    def test(self) -> NotificationTestResult:
        ...
        payload = {
            ...
            "severity": "info",
            "event_type": "test",
            "message": "Mire test notification",
            "details": {"test": True},
        }
        errors = []
        for channel in channels:
