"""Typed, per-application runtime state for Mire."""
...
MIRE_EXTENSION_KEY = "mire"
_GITHUB_RELEASE_URL = "https://api.github.com/repos/floopich/mire/releases/latest"
_T = TypeVar("_T")
...
def attach_runtime(app: Flask, runtime: MireRuntime) -> None:
    if MIRE_EXTENSION_KEY in app.extensions:
        raise RuntimeError("Mire runtime is already attached")
    app.extensions[MIRE_EXTENSION_KEY] = runtime


def get_runtime(app: Flask) -> MireRuntime:
    try:
        runtime = app.extensions[MIRE_EXTENSION_KEY]
    except KeyError as exc:
        raise RuntimeError("Mire runtime is not attached") from exc
    if not isinstance(runtime, MireRuntime):
        raise TypeError("Mire extension has an invalid runtime")
    return runtime


def current_runtime() -> MireRuntime:
