...
if (REGISTRATION_SCOPE.origin !== self.location.origin || REGISTRATION_SCOPE.pathname.slice(-1) !== '/') {
  throw new Error('Mire service worker requires a same-origin directory scope');
}
...
var CACHE_NAMESPACE = 'mire-' + encodeURIComponent(REGISTRATION_SCOPE.pathname) + '-';
...
function markOfflineShell(response) {
  ...
  return response.text().then(function(body) {
    var marker = '<meta name="mire-offline-shell" content="true">';
    var markedBody = body.indexOf('name="mire-offline-shell"') === -1
...
self.addEventListener('push', function(event) {
  ...
  event.waitUntil(self.registration.showNotification(payload.title || fallback.title, {
    ...
    tag: 'mire-' + (payload.event_type || payload.severity || 'notification'),
