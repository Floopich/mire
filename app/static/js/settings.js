...
function loadApiTokens() {
    ...
    fetch(mireUrl('/api/tokens')).then(function(r) { return r.json(); }).then(function(data) {
...
function revokeToken(id, name) {
    ...
    mireConfirm({
        ...
        return fetch(mireUrl('/api/tokens/' + id), {method: 'DELETE'});
    })
...
function testSpeedtest() {
    ...
    el.appendChild(document.createTextNode(' ' + T.testing));
    var data = getFormData();
    fetch(mireUrl('/api/test-speedtest'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
...
function clearSpeedtestCache(btn) {
    ...
    mireConfirm({
        title: T['mire.speedtest.clear_cache'] || 'Clear Cache',
        message: message,
        confirmText: T['mire.speedtest.clear_cache'] || 'Clear Cache',
        cancelText: T.cancel || 'Cancel',
        danger: true
    }).then(function(confirmed) {
        if (!confirmed) return null;
        btn.disabled = true;
        return fetch(mireUrl('/api/speedtest/cache'), { method: 'DELETE' });
    })
        .then(function(r) { return r ? r.json() : null; })
        .then(function(res) {
            if (!res) return;
            ...
            if (res.success) {
                ...
                setTimeout(function() {
                    ...
                    btn.appendChild(document.createTextNode(' ' + (T['mire.speedtest.clear_cache'] || 'Clear Cache')));
                    if (typeof lucide !== 'undefined') lucide.createIcons();
                }, 2000);
            }
        })
...
function testNotifications() {
    ...
    fetch(mireUrl('/api/notifications/test'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: '{}'
    })
...
function refreshPwaPushStatus() {
    ...
    return Promise.all([
        fetch(mireUrl('/api/notifications/pwa/status')).then(function(r) { return r.json(); }),
    ...
    refreshPwaPushStatus().then(function(status) {
        ...
        return navigator.serviceWorker.ready
            ...
            .then(function(subscription) {
                return fetch(mireUrl('/api/notifications/pwa/subscribe'), {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
    ...
    navigator.serviceWorker.ready
        ...
        .then(function(subscription) {
            ...
            return subscription.unsubscribe().then(function() {
                return fetch(mireUrl('/api/notifications/pwa/unsubscribe'), {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
...
function _saveModuleStatesIfNeeded(states) {
    ...
    return fetch(mireUrl('/api/modules/batch'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
function _saveAllSettings(options) {
    ...
    return fetch(mireUrl('/api/config'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
function backupNow() {
    ...
    el.appendChild(span);
    el.appendChild(document.createTextNode(' ' + (T.backup_creating || 'Creating backup...')));
    fetch(mireUrl('/api/backup/scheduled'), { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(res) {
        el.textContent = '';
        if (res.success) {
...
function deleteBackup(filename) {
    mireConfirm({
        ...
        if (!confirmed) return null;
        return fetch(mireUrl('/api/backup/' + encodeURIComponent(filename)), { method: 'DELETE' });
    })
    .then(function(r) { return r ? r.json() : null; })
    .then(function(res) {
        if (!res) return;
...
function openBrowseModal(opener) {
    ...
    MireModal.open(modal, {opener: opener || document.activeElement, labelledBy: 'browse-modal-title'});
    browseTo(_browsePath);
}

function closeBrowseModal() {
    MireModal.close('browse-modal');
}
...
function browseTo(path) {
    ...
    if (status) status.textContent = T.loading || 'Loading...';
    fetch(mireUrl('/api/browse?path=' + encodeURIComponent(path)))
    .then(function(r) { return r.json(); })
    .then(function(res) {
        dirs.textContent = '';
        if (res.error) {
...
function applyTheme(themeId) {
    _guardUnsaved().then(function(proceed) {
        if (!proceed) return;
        fetch(mireUrl('/api/modules/' + themeId + '/enable'), { method: 'POST' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                if (data.success) {
                    localStorage.setItem('mire-active-theme', themeId);
                    var overlay = document.getElementById('theme-preview-overlay');
                    if (overlay) overlay.style.display = 'none';
                    _previewingThemeId = null;
                    _originalStyles = {};
...
function loadSmartCaptureHistory() {
    ...
    fetch(mireUrl('/api/smart-capture/executions?limit=50'))
        .then(function(r) { return r.json(); })
        .then(function(data) {
...
function refreshModuleRegistry() {
    ...
    fetch(mireUrl('/api/modules/registry'))
        .then(function(r) { return r.json(); })
...
function _runModuleAction(e, id, action, downloadUrl) {
    ...
    fetch(mireUrl('/api/modules/' + action), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
