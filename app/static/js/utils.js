...
function exportForLLM() {
    ...
    updateExportSize();
    window.MireModal.open('export-modal');
    fetch(mireUrl('/api/export?mode=' + encodeURIComponent(mode)))
        .then(function(r) {
...
function closeExportModal() {
    exportLoadId += 1;
    window.MireModal.close('export-modal');
}
...
function openReportModal(scope) {
    ...
    renderReportScope();
    window.MireModal.open('report-modal');
...
function closeReportModal() {
    window.MireModal.close('report-modal');
    resetReportModalState();
    reportScope = null;
}
...
function syncComparisonReportState() {
    ...
    var hasComparison = !!window.__mireComparisonResult;
...
function buildReportRequestParams() {
    ...
    if (includeComparison && includeComparison.checked && window.__mireComparisonResult) {
        var cmp = window.__mireComparisonResult;
...
function generateComplaint() {
    ...
    fetch(mireUrl('/api/complaint?' + params.toString()))
        .then(function(r) {
            return r.json().then(function(data) {
                if (!r.ok || data.error) {
...
function downloadReport() {
    ...
    window.location.href = mireUrl('/api/report?' + params.toString());
}
function copyExport() {
    var textarea = document.getElementById('export-text');
...
function downloadExportMarkdown() {
    ...
    link.href = url;
    link.download = 'mire-ai-export.md';
...
function openSpeedtestSetupModal() {
    window.MireModal.open('speedtest-setup-modal');
}
function closeSpeedtestSetupModal() {
    window.MireModal.close('speedtest-setup-modal');
}
