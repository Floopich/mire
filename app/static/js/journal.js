...
var INCIDENT_ICONS = [
    ...
    {keys: ['ibpt', 'bipt', 'mediation', 'médiation', 'ombudsman', 'regulateur', 'régulateur', 'autorite', 'autorité'], icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="incident-icon"><path d="M3 21h18"/><path d="M5 21V7l7-4 7 4v14"/><path d="M9 21v-4h6v4"/></svg>', label: 'authority'},
...
function loadJournal(searchQuery) {
    ...
    var url = mireUrl('/api/journal?limit=1000');
...
function openEntryModal(entryId) {
    ...
    if (entryId) {
        ...
        fetch(mireUrl('/api/journal/' + entryId))
            .then(function(r) { return r.json(); })
            .then(function(entry) {
                idEl.value = entry.id;
...
function closeEntryModal() {
    window.MireModal.close('entry-modal');
}
...
function renderAttachments(attachments, container, incidentId) {
    ...
    attachments.forEach(function(att) {
        ...
        var thumbHtml = '';
        if (isImage) {
            thumbHtml = '<img class="attachment-thumb" src="' + mireUrl('/api/attachments/' + att.id) + '" alt="">';
        ...
        item.innerHTML = thumbHtml +
            '<div class="attachment-info">' +
                ...
                '<div class="attachment-actions">' +
                    '<a href="' + mireUrl('/api/attachments/' + att.id) + '" download title="Download">&#11015;</a>' +
...
function saveEntry() {
    ...
    var url = mireUrl(entryId ? '/api/journal/' + entryId : '/api/journal');
...
function deleteEntry() {
    ...
    mireConfirm({
        ...
        return fetch(mireUrl('/api/journal/' + entryId), {method: 'DELETE'});
    })
...
function handleEntryFileUpload(input) {
    ...
    Promise.all(uploads)
        .then(function(results) {
            ...
            input.value = '';
            // Reload attachments
            fetch(mireUrl('/api/journal/' + incidentId))
                .then(function(r) { return r.json(); })
                .then(function(inc) {
...
function uploadOneFile(incidentId, file) {
    ...
    return fetch(mireUrl('/api/journal/' + incidentId + '/attachments'), {method: 'POST', body: formData})
...
function deleteAttachment(attachmentId, incidentId) {
    fetch(mireUrl('/api/attachments/' + attachmentId), {method: 'DELETE'})
        .then(function(r) { return r.json(); })
        .then(function() {
            fetch(mireUrl('/api/journal/' + incidentId))
                .then(function(r) { return r.json(); })
...
function openImportModal() {
    ...
    setImportValidationState(T.import_validation_choose || 'Choose a CSV or Excel file. Mire will preview rows before importing.', 'info');
    _importPreviewData = null;
    window.MireModal.open('import-modal');
}

function closeImportModal() {
    window.MireModal.close('import-modal');
}
...
function handleImportFile(input) {
    ...
    fetch(mireUrl('/api/journal/import/preview'), {method: 'POST', body: formData})
        .then(function(r) { return r.json().then(function(d) { return {status: r.status, data: d}; }); })
        .then(function(res) {
            document.getElementById('import-loading').style.display = 'none';
            if (res.status >= 400) {
...
function confirmImport() {
    ...
    fetch(mireUrl('/api/journal/import/confirm'), {
        method: 'POST',
...
function deleteAllEntries() {
    ...
    mireConfirm({
        ...
        return fetch(mireUrl('/api/journal/batch'), {
            method: 'DELETE',
            headers: {'Content-Type': 'application/json'},
...
window.openIncidentTimeline = function(incidentId) {
    ...
    fetch(mireUrl('/api/incidents/' + incidentId + '/timeline'))
        .then(function(r) { return r.json(); })
        .then(function(data) {
            if (data.error) {
...
window.downloadIncidentPdf = function(incidentId, incidentName) {
    ...
    fetch(mireUrl('/api/incidents/' + incidentId + '/report?' + params.toString()))
        .then(function(r) {
        ...
        .then(function(blob) {
            ...
            a.download = 'Mire_Beschwerde_' + incidentName.replace(/[^a-zA-Z0-9]/g, '_') + '_' + new Date().toISOString().slice(0, 10) + '.pdf';
            a.click();
...
function openIncidentModal(incidentId) {
    ...
    if (incidentId) {
        ...
        deleteBtn.style.display = '';
        fetch(mireUrl('/api/incidents/' + incidentId))
            .then(function(r) { return r.json(); })
            .then(function(inc) {
                idEl.value = inc.id;
...
function closeIncidentModal() {
    window.MireModal.close('incident-container-modal');
}
...
function saveIncident() {
    ...
    var url = mireUrl(incidentId ? '/api/incidents/' + incidentId : '/api/incidents');
...
function deleteIncident() {
    ...
    mireConfirm({
        ...
        if (!confirmed) return null;
        return fetch(mireUrl('/api/incidents/' + incidentId), {method: 'DELETE'});
    })
        .then(function(r) { return r ? r.json() : null; })
        .then(function(res) {
            if (!res) return;
...
function bulkAssign() {
    ...
    fetch(mireUrl('/api/incidents/' + incidentId + '/assign'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
function bulkUnassign() {
    if (_selectedEntryIds.length === 0) return;
    fetch(mireUrl('/api/journal/unassign'), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({entry_ids: _selectedEntryIds})
    })
...
function exportJournal(fmt) {
    ...
    var url = mireUrl('/api/journal/export?format=' + fmt);
