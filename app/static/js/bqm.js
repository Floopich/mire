/* ═══ Mire BQM (Breitbandmessung Quality Monitor) ═══ */
...
function fetchBqmDates(cb) {
    fetch(mireUrl('/api/bqm/data/dates')).then(function(r) { return r.json(); }).then(function(data) {
...
function loadBqmChart(date) {
    ...
    fetch(mireUrl('/api/bqm/data/' + date))
...
function loadBqmRangeChart(start, end) {
    ...
    fetch(mireUrl('/api/bqm/data/range?start=' + encodeURIComponent(start) + '&end=' + encodeURIComponent(end)))
        .then(function(r) { return r.json(); })
        .then(function(data) {
            if (!data.points) {
...
function loadBqmLive() {
    ...
    var toggle = document.getElementById('bqm-view-toggle');
    if (toggle) toggle.style.display = 'none';
    fetch(mireUrl('/api/bqm/live')).then(function(r) {
...
function loadBqmGraph(date) {
    ...
    img.src = mireUrl('/api/bqm/image/' + date);
}
...
function importBqmCsv() {
    ...
    fetch(mireUrl('/api/bqm/import-csv'), { method: 'POST', body: formData })
        .then(function(r) {
...
function openBqmImportModal() {
    ...
    setBqmImportValidationState(T.bqm_import_validation_choose || 'Choose PNG or JPEG BQM graph images. Mire will preview dates before importing.', 'info');
    window.MireModal.open('bqm-import-modal');
}

function closeBqmImportModal() {
    window.MireModal.close('bqm-import-modal');
    // Revoke thumb URLs
...
function executeBqmImport() {
    ...
    fetch(mireUrl('/api/bqm/import'), { method: 'POST', body: fd })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            // Reload BQM calendar
            fetchBqmDates(function() {
...
function deleteBqmImages() {
    ...
    if (_bqmRangeStart && _bqmRangeEnd) {
        ...
        mireConfirm({
            ...
            return fetch(mireUrl('/api/bqm/images'), {
                method: 'DELETE',
