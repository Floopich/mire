...
function mireFormatXAxisLabel(ts, range) {
...
function mireFormatXAxisLabels(timestamps, range) {
    return (timestamps || []).map(function(ts) {
        return ts !== null && ts !== undefined ? mireFormatXAxisLabel(ts, range) : '';
    });
}

function chartXRange(u) {
    var xData = u && u.data && u.data[0] ? u.data[0] : [];
    var edgePadding = u && u._mireXEdgePadding ? u._mireXEdgePadding : DEFAULT_X_EDGE_PADDING;
        ...
        for (var i = 1; i < u.series.length; i++) {
            ...
            if (tooltipLabelCallback) {
                text = tooltipLabelCallback({
                    raw: val,
                    parsed: {y: val},
                    dataset: {label: s.label, yAxisID: s._mireAxisID},
                    dataIndex: idx
                });
            } else {
...
function renderChart(canvasId, labels, datasets, type, zones, opts) {
    ...
    if (isTrendChart) {
        cursor.sync = { key: 'mire-trends', setSeries: false };
    }
    if (zoomable) {
    ...
    chart._mireXEdgePadding = xEdgePadding;
    chart._mireParams = {labels: labels, datasets: datasets, type: type, zones: zones, opts: opts};
    ...
    chart._mireResizeObs = resizeObserver;

    /* Patch destroy to cleanup observer */
    var origDestroy = chart.destroy.bind(chart);
    chart.destroy = function() {
        if (chart._mireResizeObs) { chart._mireResizeObs.disconnect(); chart._mireResizeObs = null; }
        origDestroy();
    };
}
...
function openChartZoom(canvasId) {
    ...
    if (!src || !src._mireParams) return;
    var params = src._mireParams;
...
function closeChartZoom() {
    window.MireModal.close('chart-zoom-overlay');
    if (zoomChart) { zoomChart.destroy(); zoomChart = null; }
}
