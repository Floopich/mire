...
/* Extracted from IIFE – depends on: T, charts, renderChart, getPillValue,
   mireFormatXAxisLabels, DS_POWER_THRESHOLDS, DS_SNR_THRESHOLDS,
   US_POWER_THRESHOLDS */
...
function _fetchChannelWeatherForTimestamps(timestamps) {
    ...
    var url = mireUrl('/api/weather/range?start=' + encodeURIComponent(wr.start) + '&end=' + encodeURIComponent(wr.end));
...
function _renderChannelTimelineCharts() {
    ...
    var xLabels = mireFormatXAxisLabels(data.map(function(d) { return d.timestamp || ''; }), days);
    ...
    if (mods.length === 0) {
        ...
        var modLabels = mireFormatXAxisLabels(mods.map(function(d) { return d.timestamp; }), days);
...
function loadChannelTimeline() {
    ...
    fetch(mireUrl('/api/channel-history?' + identityParam + '&direction=' + direction + '&range=' + encodeURIComponent(days)))
        .then(function(r) { return r.json(); })
        .then(function(data) {
...
function _renderCompareCharts() {
    ...
    var xLabels = mireFormatXAxisLabels(timestamps, days);
...
function loadCompareCharts() {
    ...
    fetch(mireUrl('/api/channel-compare?' + identityName + '=' + encodeURIComponent(identities) + '&direction=' + dir + '&range=' + encodeURIComponent(days)))
        .then(function(r) { return r.json(); })
        .then(function(data) {
