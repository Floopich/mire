var dashboardBootstrapElement = document.getElementById('mire-dashboard-bootstrap');
var dashboardBootstrap = MireBrowserContracts.parseDashboardBootstrapText(
...
(function() {
    var LAST_KNOWN_STORAGE_KEY = 'mire:last-known-dashboard-shell';
    ...
    function updateOfflineStatus() {
        var offlineMarker = document.querySelector('meta[name="mire-offline-shell"][content="true"]');
        var offline = !navigator.onLine || !!offlineMarker || window.__MIRE_OFFLINE_SHELL__ === true;
        ...
        if (lastKnown) {
            ...
            lastKnown.textContent = stored ? 'Last-known shell: ' + MireBrowserContracts.formatLastKnownTimestamp(stored) : 'Last-known shell timestamp unavailable';
        }
...
(function() {
    ...
    function switchView(view, skipHash) {
        ...
        if (view === 'live') {
            ...
            /* Generic module init: mod-mire-comparison → initComparison */
            var parts = view.replace('mod-mire-', '').split('-');
    ...
    function refreshData() {
        fetch(window.location.href)
            .then(function(r) {
                if (r.headers && r.headers.get('X-Mire-Offline-Shell') === 'true') {
                    window.__MIRE_OFFLINE_SHELL__ = true;
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', function() {
            ...
            fetch(mireUrl('/api/poll'), {method: 'POST'})
                ...
                .finally(function() {
                    ...
                    setTimeout(function() {
                        if (navigator.onLine && window.__MIRE_OFFLINE_SHELL__ !== true) refreshBtn.disabled = false;
                    }, 10000);
                });
        });
    }
