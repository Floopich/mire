var setupBootstrapElement = document.getElementById('mire-setup-bootstrap');
var setupBootstrap = MireBrowserContracts.parseSetupBootstrapText(
    setupBootstrapElement && setupBootstrapElement.textContent
);
var SETUP_INDEX_URL = mireUrl(setupBootstrap.indexUrl);
var SETUP_LOGIN_URL = mireUrl(setupBootstrap.loginUrl);
let currentStep = 1;
...
async function testConnection() {
    ...
    try {
        ...
        var response = await fetch(mireUrl('/api/test-modem'), {
            method: 'POST',
...
function toggleUsernameField() {
    ...
    var state = MireBrowserContracts.selectSetupDriverState(
...
async function validateRestoreFile() {
    ...
    try {
        var response = await fetch(mireUrl('/api/restore/validate'), { method: 'POST', body: formData });
...
async function doRestore() {
    ...
    try {
        var response = await fetch(mireUrl('/api/restore'), { method: 'POST', body: formData });
        var res = await response.json();
