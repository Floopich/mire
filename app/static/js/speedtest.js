...
function toggleSpeedtestSignal(btn) {
    ...
    if (_signalCache[id]) {
        ...
        fetch(mireUrl('/api/speedtest/' + id + '/signal'))
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    if (_enrichedCache[id]) {
        ...
        fetch(mireUrl('/api/speedtest/' + id))
            .then(function(r) { return r.json(); })
            .then(function(data) {
...
function runSpeedtest() {
    ...
    fetch(mireUrl('/api/speedtest?count=1'))
        .then(function(r) { return r.json(); })
        .then(function(latest) {
            var lastId = (latest && latest.length > 0) ? latest[0].id : 0;
            return fetch(mireUrl('/api/speedtest/run'), {method: 'POST'})
                .then(function(r) {
                    return r.json()
