...
def _registry_exists(conn: sqlite3.Connection) -> bool:
    return table_exists(conn, "_mire_migrations")


def _ensure_registry(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS _mire_migrations (
            id TEXT PRIMARY KEY,
...
def run_migrations(db_path: str, migrations: Sequence[Migration]) -> list[str]:
    ...
    with open_read(db_path) as conn:
        if _registry_exists(conn):
            recorded = {
                row[0]
                for row in conn.execute("SELECT id FROM _mire_migrations")
            }
    ...
    for migration in migrations:
        with write_transaction(db_path) as conn:
            if conn.execute(
                "SELECT 1 FROM _mire_migrations WHERE id=?",
            ...
            conn.execute(
                "INSERT INTO _mire_migrations (id, applied_at) VALUES (?, ?)",
                (
                    migration.migration_id,
...
def _meta_version_applied(conn: sqlite3.Connection) -> bool:
    if not table_exists(conn, "_mire_meta"):
        return False
    row = conn.execute(
...
def _apply_meta_version(conn: sqlite3.Connection) -> None:
    conn.execute(
        "INSERT INTO _mire_meta (key, value) VALUES ('schema_version', ?) "
