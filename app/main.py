...
from .runtime import MireRuntime, get_runtime
...
if os.environ.get("MIRE_AUDIT_JSON", "").strip() == "1":
...
def _get_modem_config_key(config_mgr):
    ...
    return (
        config_mgr.get("modem_type", "generic"),
...
def polling_loop(config_mgr, storage, stop_event, runtime: MireRuntime):
    ...
    if module_loader:
        for mod in module_loader.get_enabled_modules():
            if mod.publisher_class and mod.id == 'mire.mqtt':
    ...
    if mqtt_cls and config_mgr.is_mqtt_configured():
        ...
        log.warning("MQTT configured but mire.mqtt module not available (disabled?)")
    else:
    ...
    # Wire STT adapter if STT configured
