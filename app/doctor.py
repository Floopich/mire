"""Offline self-hosted diagnostics for Mire.
...
def _check_runtime(environ: Mapping[str, str]) -> list[CheckResult]:
    ...
    return [
        CheckResult(
            ...
            category="runtime",
            status="pass",
            message="Mire runtime information collected",
            details={
...
def _check_configured_mode(raw_config: Mapping[str, Any], environ: Mapping[str, str]) -> CheckResult:
    ...
    configured = "modem_type" in raw_config or bool(environ.get("MODEM_TYPE"))
    ...
    if not configured:
        return CheckResult(
            ...
            category="configuration",
            status="warn",
            message="Mire setup does not appear to be completed yet",
            core=False,
            details=details,
        )
    return CheckResult(
        ...
        category="configuration",
        status="pass",
        message="Mire setup state is present",
        details=details,
    )
...
def format_human(report: Mapping[str, Any], color: bool = True) -> str:
    ...
    lines = [
        f"Mire doctor v{report.get('doctor_version')} (app {report.get('app_version')})",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run offline Mire self-hosted diagnostics.")
    parser.add_argument("--json", action="store_true", help="emit machine-readable JSON")
    parser.add_argument("--data-dir", default=None, help="Mire data directory (default: DATA_DIR or /data)")
    ...
    try:
        ...
        if args.json:
            ...
            print(f"Mire doctor failed: {payload['error']}", file=sys.stderr)
        return 2
