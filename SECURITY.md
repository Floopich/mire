...
If you discover a security vulnerability in Mire, please report it privately:

### Preferred Method: GitHub Security Advisories

1. Go to https://github.com/floopich/mire/security/advisories
...
Mire is designed to run **100% locally** on your network:
...
Mire can show maintainer notices for important project, upgrade, or safety information. These notices are designed for self-hosted environments and follow the same local-first privacy model as the rest of Mire.

Current notice behavior:

- Notices are bundled with the installed Mire release.
...
- Dismissals are stored locally in the Mire configuration by stable notice ID.
...
- Mire does not fetch a remote notice feed or render remote HTML for maintainer notices.

Release update checks are also local-first and operator-controlled:

- `update_check_enabled` / `UPDATE_CHECK_ENABLED` controls whether Mire checks the public GitHub Releases API for newer Mire versions.
...
- Mire must not upload private payloads, identifiers, analytics, or modem data while checking notices.
...
The session key is stored in `DATA_DIR/.session_key`, so normal restarts with the same data directory preserve valid logins. `DATA_DIR/.auth_state` stores only a keyed fingerprint of the effective authentication state so that observed password removal and later same-value re-enablement cannot revive an old cookie. Both files are atomically written with mode `0600`. Cookies created before password binding was introduced do not contain the required marker and require a one-time login after upgrade. Mire does not store the admin password, its hash, or a reusable unkeyed password-derived value in either the cookie or auth-state file.

For HTTPS deployments, enable trusted reverse-proxy mode with `REVERSE_PROXY`; Mire then keeps the session cookie `Secure`, so it is sent only over HTTPS. Do not expose an authenticated instance over unencrypted remote HTTP.

### Reverse-Proxy Path Prefixes

Mire Core has two generic ways to learn an external URL mount after a reverse
...
The edge proxy must accept Mire only on the configured same-origin mount,
...
A Home Assistant-shaped mount does not give Mire Core a Home Assistant
identity or session. Home Assistant may authenticate access to its Ingress
gateway, but Mire keeps its own authentication boundary. See the
...
Mire sets the following headers on every response:
...
If you run Mire behind a reverse proxy, the proxy does **not** need to duplicate these headers.
...
**Structured JSON output** can be enabled by setting `MIRE_AUDIT_JSON=1`. This produces log lines like:
...
1. **Keep Mire updated:** Run `docker pull` regularly
2. **Restrict network access:** Use a reverse proxy with HTTPS for remote access
3. **Use strong modem passwords:** Mire inherits your modem's security
4. **Review audit logs:** Check `docker logs mire` or enable JSON audit logging for structured analysis
...
Mire uses Python libraries from PyPI. We monitor dependencies for known vulnerabilities:
...
Thank you for helping keep Mire and its users safe!
