...
class TestDownstreamChannelMetrics:
    def test_ds_power_dbmv(self):
        ...
        assert _has_metric(out, 'mire_downstream_power_dbmv{channel_id="1",frequency="474"} 3.0')

    def test_ds_snr_db(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_snr_db{channel_id="1",frequency="474"} 35.0')

    def test_ds_corrected_errors_total(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_corrected_errors_total{channel_id="1",frequency="474"} 100')

    def test_ds_uncorrected_errors_total(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_uncorrected_errors_total{channel_id="1",frequency="474"} 5')

    def test_ds_modulation_256qam(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_modulation{channel_id="1",frequency="474"} 256')
    ...
    def test_ds_snr_none_omits_line(self):
        ...
        assert not _has_metric_approx(out, 'mire_downstream_snr_db{channel_id="5",frequency="474"}')
    ...
    def test_ds_modulation_ofdm_omits_line(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert not _has_metric_approx(out, 'mire_downstream_modulation{channel_id="7",frequency="474"}')

    def test_multiple_ds_channels_separate_lines(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_power_dbmv{channel_id="1",frequency="474"} 3.0')
        assert _has_metric(out, 'mire_downstream_power_dbmv{channel_id="2",frequency="482"} -1.5')
    ...
    def test_ds_has_help_comment(self):
        ...
        assert any("# HELP mire_downstream_power_dbmv" in l for l in lines)

    def test_ds_has_type_comment(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        lines = out.splitlines()
        assert any("# TYPE mire_downstream_power_dbmv" in l for l in lines)
...
class TestUpstreamChannelMetrics:
    def test_us_power_dbmv(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_upstream_power_dbmv{channel_id="1",frequency="30"} 42.0')

    def test_us_modulation_64qam(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        assert _has_metric(out, 'mire_upstream_modulation{channel_id="1",frequency="30"} 64')
    ...
    def test_us_power_none_omits_line(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert not _has_metric_approx(out, 'mire_upstream_power_dbmv{channel_id="3",frequency="30"}')
    ...
    def test_multiple_us_channels_separate_lines(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert _has_metric(out, 'mire_upstream_power_dbmv{channel_id="1",frequency="30"} 42.0')
        assert _has_metric(out, 'mire_upstream_power_dbmv{channel_id="2",frequency="38"} 45.0')
    ...
    def test_frequency_label_normalizes_mhz_values(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert _has_metric(out, 'mire_downstream_power_dbmv{channel_id="9",frequency="114.0"} 2.5')
        assert _has_metric(out, 'mire_upstream_modulation{channel_id="2",frequency="44"} 64')

    def test_us_has_help_comment(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_upstream_power_dbmv" in l for l in lines)

    def test_us_has_type_comment(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        lines = out.splitlines()
        assert any("# TYPE mire_upstream_power_dbmv" in l for l in lines)
...
class TestSummaryMetrics:
    ...
    def test_channel_counts_zero_for_empty_channels(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert _has_metric(out, "mire_downstream_channels_total 0")
        assert _has_metric(out, "mire_upstream_channels_total 0")
...
class TestDeviceInfoMetrics:
    def test_device_info_metric(self):
        ...
        assert _has_metric(out, 'mire_device_info{model="FRITZ!Box 6690",sw_version="7.57"} 1')

    def test_device_uptime_seconds(self):
        out = format_metrics(None, DEVICE_INFO_FULL, None, 0.0)
        assert _has_metric(out, "mire_device_uptime_seconds 86400")
    ...
    def test_device_info_without_uptime_omits_uptime_line(self):
        ...
        assert not _has_metric_approx(out, "mire_device_uptime_seconds")

    def test_device_info_none_no_device_lines(self):
        out = format_metrics(None, None, None, 0.0)
        assert not _has_metric_approx(out, "mire_device_info")
        assert not _has_metric_approx(out, "mire_device_uptime_seconds")

    def test_device_info_has_help_comment(self):
        out = format_metrics(None, DEVICE_INFO_FULL, None, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_device_info" in l for l in lines)

    def test_device_info_has_type_comment(self):
        out = format_metrics(None, DEVICE_INFO_FULL, None, 0.0)
        lines = out.splitlines()
        assert any("# TYPE mire_device_info" in l for l in lines)
    ...
    def test_device_info_excludes_mutable_fields(self):
        ...
        device_info_lines = [
            ...
            if line.startswith("mire_device_info{")
        ]
        assert device_info_lines, "mire_device_info line missing"
    ...
    def test_docsis_online_is_one_when_online(self):
        ...
        out = format_metrics(None, info, None, 0.0)
        assert _has_metric(out, "mire_docsis_online 1")
    ...
    def test_docsis_online_is_zero_for_other_statuses(self):
        for status in ("offline", "scanning frequencies", "firmware update"):
            ...
            out = format_metrics(None, info, None, 0.0)
            assert _has_metric(out, "mire_docsis_online 0"), f"status={status}"

    def test_docsis_online_omitted_when_status_missing(self):
        info = {"model": "TG6442VF"}
        out = format_metrics(None, info, None, 0.0)
        assert not _has_metric_approx(out, "mire_docsis_online")

    def test_last_reboot_reason_info_emitted_with_label(self):
        info = {"model": "TG6442VF", "reboot_reason": "power on"}
        out = format_metrics(None, info, None, 0.0)
        assert _has_metric(out, 'mire_last_reboot_reason_info{reason="power on"} 1')

    def test_last_reboot_reason_info_omitted_when_missing(self):
        info = {"model": "TG6442VF"}
        out = format_metrics(None, info, None, 0.0)
        assert not _has_metric_approx(out, "mire_last_reboot_reason_info")

    def test_wan_ipv4_info_emitted_with_label(self):
        info = {"model": "TG6442VF", "wan_ipv4": "192.0.2.1"}
        out = format_metrics(None, info, None, 0.0)
        assert _has_metric(out, 'mire_device_wan_ipv4_info{wan_ipv4="192.0.2.1"} 1')

    def test_wan_ipv4_info_omitted_when_missing(self):
        info = {"model": "TG6442VF"}
        out = format_metrics(None, info, None, 0.0)
        assert not _has_metric_approx(out, "mire_device_wan_ipv4_info")
    ...
    def test_wan_ipv6_info_emitted_with_label(self):
        ...
        out = format_metrics(None, info, None, 0.0)
        assert _has_metric(out, 'mire_device_wan_ipv6_info{wan_ipv6="2001:db8::1"} 1')

    def test_wan_ipv6_info_omitted_when_missing(self):
        info = {"model": "TG6442VF"}
        out = format_metrics(None, info, None, 0.0)
        assert not _has_metric_approx(out, "mire_device_wan_ipv6_info")
    ...
    def test_wan_label_values_are_escaped(self):
        ...
        assert _has_metric(
            out,
            'mire_device_wan_ipv4_info{wan_ipv4="bogus\\"value"} 1',
        )
        assert _has_metric(
            out,
            'mire_device_wan_ipv6_info{wan_ipv6="line1\\nline2"} 1',
        )
    ...
    def test_label_values_are_escaped(self):
        ...
        assert _has_metric(
            out,
            'mire_device_info{model="Model \\"Gamma\\"\\\\beta"} 1',
        )
        assert _has_metric(
            out,
            'mire_last_reboot_reason_info{reason="line1\\nline2"} 1',
        )
...
class TestConnectionInfoMetrics:
    def test_max_downstream_kbps(self):
        ...
        assert _has_metric(out, "mire_connection_max_downstream_kbps 1000000")

    def test_max_upstream_kbps(self):
        out = format_metrics(None, None, CONNECTION_INFO_FULL, 0.0)
        assert _has_metric(out, "mire_connection_max_upstream_kbps 50000")

    def test_connection_info_none_no_connection_lines(self):
        out = format_metrics(None, None, None, 0.0)
        assert not _has_metric_approx(out, "mire_connection_max_downstream_kbps")
        assert not _has_metric_approx(out, "mire_connection_max_upstream_kbps")

    def test_connection_has_help_comment(self):
        out = format_metrics(None, None, CONNECTION_INFO_FULL, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_connection_max_downstream_kbps" in l for l in lines)

    def test_connection_has_type_comment(self):
        out = format_metrics(None, None, CONNECTION_INFO_FULL, 0.0)
        lines = out.splitlines()
        assert any("# TYPE mire_connection_max_downstream_kbps" in l for l in lines)
...
class TestPollTimestampMetrics:
    def test_last_poll_timestamp(self):
        ...
        assert _has_metric(out, "mire_last_poll_timestamp_seconds 1709568000.0")

    def test_last_poll_timestamp_zero(self):
        out = format_metrics(None, None, None, 0.0)
        assert _has_metric(out, "mire_last_poll_timestamp_seconds 0.0")

    def test_last_poll_has_help_comment(self):
        out = format_metrics(None, None, None, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_last_poll_timestamp_seconds" in l for l in lines)

    def test_last_poll_has_type_comment(self):
        out = format_metrics(None, None, None, 0.0)
        lines = out.splitlines()
        assert any("# TYPE mire_last_poll_timestamp_seconds" in l for l in lines)
...
class TestOutputFormat:
    ...
    def test_health_has_help_and_type(self):
        out = format_metrics(None, None, None, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_health_status" in l for l in lines)
        assert any("# TYPE mire_health_status" in l for l in lines)

    def test_channel_counts_have_help_and_type(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        lines = out.splitlines()
        assert any("# HELP mire_downstream_channels_total" in l for l in lines)
        assert any("# TYPE mire_downstream_channels_total" in l for l in lines)

    def test_counters_have_counter_type(self):
        out = format_metrics(ANALYSIS_FULL, None, None, 0.0)
        lines = out.splitlines()
        corr_type = next(
            (l for l in lines if "# TYPE mire_downstream_corrected_errors_total" in l), None
        )
        uncorr_type = next(
            (l for l in lines if "# TYPE mire_downstream_uncorrected_errors_total" in l), None
        )
...
class TestEdgeCases:
    ...
    def test_empty_channels_no_per_channel_lines(self):
        ...
        out = format_metrics(analysis, None, None, 0.0)
        assert not _has_metric_approx(out, 'mire_downstream_power_dbmv{')
        assert not _has_metric_approx(out, 'mire_upstream_power_dbmv{')
