...
def test_manifest_is_canonical_and_redacted(tmp_path):
    ...
    manifest = canonical_manifest(app, app.extensions.get("mire_module_loader"))
    encoded = json.dumps(manifest, sort_keys=True)

    assert manifest["version"] == 1
    assert manifest_fingerprint(manifest) == app.extensions["mire_registration_fingerprint"]
...
print(app.extensions['mire_registration_fingerprint'])
"""
    fingerprints = []
