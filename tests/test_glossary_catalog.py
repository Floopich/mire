...
def test_core_glossary_contains_only_docsis_terms_and_mire_features():
    ...
    assert set(terms) == expected_docsis_terms | expected_mire_features
    for term in terms.values():
        assert term["category"] in {"docsis_terms", "mire_features"}
        assert term["title"]
        assert term["aliases"]
...
def test_categories_are_localized_and_used_by_terms():
    ...
    assert set(categories) == {"docsis_terms", "mire_features"}
...
def test_glossary_domain_semantics_are_pinned_to_mire_behavior():
