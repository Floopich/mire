...
def test_disabled_builtin_still_registers_private_metadata(tmp_path, monkeypatch):
    ...
    loader = ModuleLoader(
        ...
        disabled_ids={"mire.reports_test"},
    )

    modules = loader.load_all()

    reports_module = next(mod for mod in modules if mod.id == "mire.reports_test")
...
def test_reports_settings_panel_renders_saved_values(
    ...
    panel = soup.select_one("#panel-mod-mire_reports")
