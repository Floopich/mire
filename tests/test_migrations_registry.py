    ...
    with sqlite3.connect(db_path) as conn:
        return [
            ...
            for row in conn.execute(
                "SELECT id FROM _mire_migrations ORDER BY applied_at, id"
            )
        ]
...
def test_failed_migration_rolls_back_shape_and_registry_then_retries(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        assert conn.execute(
            "SELECT 1 FROM _mire_migrations WHERE id='test-0001-retry'"
        ).fetchone() is None
...
def test_fresh_core_reaches_head_and_preserves_schema_version(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        assert conn.execute(
            "SELECT value FROM _mire_meta WHERE key='schema_version'"
        ).fetchone() == ("2",)
        columns = {row[1] for row in conn.execute("PRAGMA table_info(snapshots)")}
        assert {"raw_json", "analysis_meta_json"} <= columns
...
def test_hybrid_incidents_shape_is_warned_stamped_and_untouched(tmp_path, caplog):
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "CREATE TABLE incidents (id INTEGER PRIMARY KEY, title TEXT NOT NULL)"
        )
        conn.execute(
            "CREATE TABLE journal_entries (id INTEGER PRIMARY KEY, title TEXT NOT NULL)"
        )
        conn.execute("INSERT INTO incidents VALUES (1, 'old')")
        conn.execute("INSERT INTO journal_entries VALUES (2, 'new')")
    before = data_digest(db_path)

    with caplog.at_level(logging.WARNING):
        migrations.run_migrations(str(db_path), migrations.CORE_MIGRATIONS)

    after = data_digest(db_path, exclude={"_mire_meta", "snapshots", "device_state", "events", "weather_data", "api_tokens", "pwa_push_subscriptions", "smart_capture_executions"})
    ...
    with sqlite3.connect(db_path) as conn:
        assert conn.execute("SELECT * FROM incidents").fetchall() == [(1, "old")]
        assert conn.execute("SELECT * FROM journal_entries").fetchall() == [(2, "new")]
