"""Behavioral regression checks for Mire PWA mount and offline contracts."""
  ...
  return {
    ...
    match: async function(key) {
      if (input.cachedShell && name === context.SHELL_CACHE && key === context.MOUNT_ROOT) {
        return new Response('<html><head></head><body>Cached Mire</body></html>', {
          status: 200,
...
const context = {
  ...
  fetch: async function() {
    ...
    return new Response('<html><head></head><body>Mire</body></html>', {
      status: 200,
      headers: {'Content-Type': 'text/html'}
    });
  }
...
async function probeFetch(probe) {
  ...
  if (input.inspectResponses) {
    ...
    result.offlineHeader = response ? response.headers.get('X-Mire-Offline-Shell') : null;
    result.offlineMarked = response ? (await response.text()).includes('name="mire-offline-shell"') : false;
  }
  return result;
}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "https://example.test/mire/static/manifest.json",
            "https://example.test/mire/",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "https://example.test/mire/",
            "/mire",
            ["/mire/", "/mire/?source=pwa"],
            [
                "/mire/static/manifest.json",
                "/mire/static/logo.svg",
                "/mire/static/icon.png",
            ],
        ),
    ],
)
...
def test_service_worker_classifies_requests_identically_at_root_and_prefix(
    ...
    probes = [
        ...
        {"method": "GET", "url": "https://outside.test/mire/", "mode": "navigate"},
...
def test_cache_names_and_activation_cleanup_are_mount_isolated():
    ...
    prefixed = _run_service_worker("https://example.test/mire/")
    keys = [
        prefixed["shellCache"],
...
def test_offline_shell_uses_the_mounted_fallback_and_explicit_marker():
    result = _run_service_worker(
        ...
        fetches=[
            {
                "method": "GET",
                "url": "https://example.test/mire/?source=pwa",
                "mode": "navigate",
            }
        ],
    )
...
@pytest.mark.parametrize(
    ...
    [
        {
            "method": "GET",
            "url": "https://example.test/mire/dashboard",
            "mode": "navigate",
        },
        {
            "method": "GET",
            "url": "https://example.test/mire/static/app.js",
        },
    ],
    ids=["shell", "static"],
)
...
def test_notification_targets_are_mounted_once_and_unsafe_targets_fall_back():
    fallback = "/mire/?source=pwa#events"
    targets = [
        "/?source=pwa#events",
        "/settings#notifications",
        "/mire/settings#notifications",
        "https://example.test/mire/events?id=1#details",
        "https://evil.test/mire/events",
        "//evil.test/mire/events",
        "https://example.test/admin",
        "https://example.test/mire-other/events",
        "/mire/../admin",
        "/mire/%2e%2e/admin",
        "/mire/%252e%252e/admin",
        "/mire/%GG",
        "/mire\\admin",
        "/mire/\u0000admin",
    ]
...
def test_push_assets_and_notification_click_clients_stay_inside_mount():
    ...
    result = _run_service_worker(
        ...
        clients=[
            ...
            "https://example.test/mire/dashboard",
        ],
    ...
    assert result["operations"]["notifications"] == [
        {
            "title": "Signal",
            "options": {
                "body": "Open Mire for the latest signal status.",
                "icon": "/mire/static/icon.png",
                "badge": "/mire/static/icon.png",
                "tag": "mire-info",
                "data": {"url": fallback},
            },
        }
...
def test_index_template_exposes_honest_offline_state_and_scoped_pwa_lifecycle():
    ...
    assert "__MIRE_OFFLINE_SHELL__" in dashboard
    assert 'meta[name="mire-offline-shell"][content="true"]' in dashboard
    assert '<meta name="mire-offline-shell" content="true">' in service_worker
    assert "<script>" not in service_worker
    assert "X-Mire-Offline-Shell" in dashboard
    assert "navigator.onLine && window.__MIRE_OFFLINE_SHELL__ !== true" in dashboard
    assert "registration.scope === mireServiceWorkerScope" in registration
    assert "key.indexOf(mireCacheNamespace) === 0" in registration
    assert "register(mireUrl('/sw.js'), { scope: mireUrl('/') })" in registration
