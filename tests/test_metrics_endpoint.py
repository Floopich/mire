...
class TestMetricsEndpoint:
    ...
    def test_no_data_returns_health_unknown(self, metrics_client):
        ...
        assert "mire_health_status 4" in body
    ...
    def test_with_data_returns_channel_metrics(self, metrics_client_with_data):
        ...
        assert resp.status_code == 200
        body = resp.data.decode("utf-8")
        assert "mire_downstream_power_dbmv" in body
        assert "mire_upstream_power_dbmv" in body
        assert "mire_health_status 0" in body
    ...
    def test_last_poll_timestamp_present(self, metrics_client):
        """Response contains mire_last_poll_timestamp_seconds metric."""
        resp = metrics_client.get("/metrics")
        body = resp.data.decode("utf-8")
        assert "mire_last_poll_timestamp_seconds" in body
    ...
    def test_token_protection_allows_valid_token(self, protected_metrics_client):
        _, token = protected_metrics_client._mire_storage.create_api_token("prometheus")
