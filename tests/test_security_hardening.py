...
from app.theme_registry import _is_trusted_url
...
class TestThemeURLValidation:
    ...
    def test_download_rejects_untrusted_url(self, tmp_path):
        """Le telechargement doit refuser une URL de source non fiable."""
    ...
    def test_download_skips_untrusted_file_urls(self, mock_urlopen, tmp_path):
        ...
        target = str(tmp_path / "test_module")
        result = download_github_directory(
            "https://api.github.com/repos/user/repo/contents/module",
            target,
        )
        # Le fichier a l'URL non fiable ne doit jamais atterrir sur le disque.
        import os
        written = os.listdir(target) if os.path.isdir(target) else []
        assert "manifest.json" not in written
...
def client():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        cfg.save({"admin_password": "testpass", "modem_type": "generic"})
