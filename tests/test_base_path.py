...
def test_trusted_prefix_uses_proxyfix_right_to_left_hop_selection():
    ...
    response = client.get(
        "/",
        headers={"X-Forwarded-Prefix": "/mire, /untrusted-edge"},
    )

    assert response.status_code == 200
    assert response.get_json()["script_name"] == "/mire"
...
def test_malformed_unselected_forwarded_prefix_is_ignored():
    ...
    response = client.get(
        "/",
        headers={"X-Forwarded-Prefix": "/bad%2f.., /mire"},
    )
...
def test_missing_trusted_forwarded_prefix_with_fixed_base_path_returns_400():
    client = _make_app(base_path="/mire", trusted_hops="1").test_client()
...
def test_proxy_stripped_path_info_is_never_rewritten(path):
    client = _make_app(base_path="/mire").test_client()
...
@pytest.mark.parametrize(
    ...
    [
        ("/mire", None, {}, "/mire/"),
        (None, "1", {"X-Forwarded-Prefix": "/mire"}, "/mire/"),
        (None, None, {}, "/"),
        ("/", None, {}, "/"),
    ],
)
def test_session_cookie_is_scoped_to_request_script_name(
    ...
    if expected_path == "/mire/":
        assert "Path=/mire-other" not in response.headers["Set-Cookie"]
