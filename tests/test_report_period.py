...
@pytest.mark.parametrize(
    ...
    [
        # Bornes critiques du profil VOO (empiriques, non normatives) :
        # descendant +/-15 dBmV, montant 25 a 52 dBmV, snr ofdm 24.5 dB.
        ("ds", {"type": "SC-QAM", "modulation": "256QAM"}, "power", 20.0, "power critical high", "ds_power_high", 15.0),
        ("ds", {"type": "SC-QAM", "modulation": "256QAM"}, "power", -20.0, "power critical low", "ds_power_low", -15.0),
        ("ds", {"type": "OFDM", "docsis_version": "3.1", "modulation": "4096QAM"}, "snr", 20.0, "snr critical", "snr_low", 24.5),
        ("us", {"type": "SC-QAM", "docsis_version": "3.0"}, "power", 60.0, "power critical high", "us_power_high", 52.0),
        ("us", {"type": "OFDMA", "docsis_version": "3.1"}, "power", 20.0, "power critical low", "us_power_low", 25.0),
    ],
)
...
def test_exact_numeric_provenance_wins_over_neutral_candidate(monkeypatch):
    ...
    assert note["spec_max"] == 15.0   # borne critique descendante VOO
...
def test_pdf_and_text_complaint_use_one_period_aggregate_per_artifact():
    ...
    expected = (
        ...
        "maximum (15.0 dBmV) by 100%. Observed at 2026-05-01T01:02:03Z UTC."
    )
    assert expected in " ".join(pdf_text.split())
    assert expected in complaint


def test_empty_complaint_is_neutral_without_docsis_data():
    from app.modules.reports.report import generate_complaint_text
