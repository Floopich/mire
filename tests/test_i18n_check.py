...
SPEC = importlib.util.spec_from_file_location("mire_i18n_check", SCRIPT_PATH)
...
def test_generate_writes_only_core_template(tmp_path, monkeypatch):
    ...
    (core / "en.json").write_text(json.dumps({"_meta": {"language_name": "English", "flag": "gb"}, "title": "Mire"}), encoding="utf-8")
...
def test_signal_family_status_and_signal_quality_copy_cover_all_core_locales():
    ...
    assert len(locale_paths) == 4   # Mire ne livre que de/en/fr/nl
