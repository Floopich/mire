...
test('settings bootstrap validates modules, cooldown fallback, secrets, and time data', () => {
    const base = {
        ...
        modules: [{id: 'mire.example', labelKey: 'mire.example.title', name: 'Example'}],
        serverOffsetMin: 60,
...
test('service-worker policy is prefix-scoped and only disables local development by default', () => {
    assert.deepEqual(
        contracts.computeServiceWorkerPolicy('localhost', '', 'https://example.test/mire/'),
        {action: 'cleanup', scopeHref: 'https://example.test/mire/', cacheNamespace: 'mire-%2Fmire%2F-'}
    );
    assert.equal(
        contracts.computeServiceWorkerPolicy('127.0.0.1', '?enable-sw-test=1', 'https://example.test/mire/').action,
        'register'
    );
    assert.equal(
        contracts.computeServiceWorkerPolicy('mire.test', '', 'https://mire.test/').cacheNamespace,
        'mire-%2F-'
    );
    for (const unsafe of ['not a url', 'javascript:alert(1)', 'https://example.test/mire/?query=1']) {
