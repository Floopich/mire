...
class TestIndexRoute:
    ...
    def test_home_signal_family_card_status_uses_composite_health_without_cause_suffix(self, client, sample_analysis):
        ...
        # Profil VOO : 51.5 dBmV depasse le plafond de 51 -> avertissement.
        ofdma["power"].update({"available": True, "avg": 51.5, "min": 51.5, "max": 51.5, "health": "warning"})
        ofdma["modulation"].update({"value": "32QAM", "distinct": ["32QAM"], "health": "critical"})
        sample_analysis["summary"]["us_ofdma_power_avg"] = 51.5
        update_state(analysis=sample_analysis)
        ...
        assert "51.5<span class=\"unit\">dBmV</span>" in card
...
class TestIndexSegmentUtilizationVisibility:
    def test_index_never_renders_the_removed_segment_tab(self, client, config_mgr, sample_analysis):
        """Non-regression : la vue Segment Utilization (FRITZ!Box) a ete retiree."""
