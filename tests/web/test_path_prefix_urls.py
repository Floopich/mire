...
def _browser_bootstrap(html: str) -> tuple[dict[str, str], BeautifulSoup]:
    soup = BeautifulSoup(html, "html.parser")
    element = soup.find("script", id="mire-url-bootstrap")
...
def _assert_url_helper_precedes_application_scripts(html: str, prefix: str) -> None:
    ...
    bootstrap_index = next(
        index
        for index, script in enumerate(scripts)
        if script.get("id") == "mire-url-bootstrap"
    )
...
def test_dashboard_and_settings_render_prefix_aware_navigation_and_assets(
    ...
    for html in (dashboard_html, settings_html):
        ...
        assert all(value.startswith("/mire/") for value in root_attributes)

    assert 'href="/mire/settings"' in dashboard_html
    assert 'href="/mire/settings#support"' in dashboard_html
    assert (
        'href="/mire/api/events/export.csv?exclude_operational=true"'
        in dashboard_html
    )
    assert (
    ...
    assert 'src="/mire/static/js/settings.js?v=' in settings_html
...
def test_login_and_setup_render_prefix_aware_assets_forms_and_navigation(tmp_path):
    ...
    assert all(
        value.startswith("/mire/")
        for value in _application_root_attributes(login_html)
    )
    assert 'href="/mire/static/css/fonts.css?v=' in login_html
    ...
    assert 'action="/mire/api/config"' in setup_html
    setup_soup = BeautifulSoup(setup_html, "html.parser")
    setup_bootstrap = setup_soup.find("script", id="mire-setup-bootstrap")
    ...
    assert setup_payload["indexUrl"] == "/mire/"
    assert setup_payload["loginUrl"] == "/mire/login"
...
def test_setup_bootstrap_tojson_keeps_hostile_translation_inert(tmp_path, monkeypatch):
    ...
    soup = BeautifulSoup(html, "html.parser")
    bootstrap = soup.find("script", id="mire-setup-bootstrap")
...
def test_auth_setup_glossary_and_backup_redirects_preserve_script_name(tmp_path):
    ...
    assert auth_redirect.headers["Location"] == "/mire/login"
    assert restore_redirect.headers["Location"] == "/mire/login"
    assert login_redirect.headers["Location"] == "/mire/"
    assert 'action="/mire/logout"' in authenticated_dashboard.get_data(as_text=True)
    assert logout_redirect.headers["Location"] == "/mire/login"
    ...
    assert setup_redirect.headers["Location"] == "/mire/"
    assert (
        glossary_redirect.headers["Location"]
        == "/mire/?lang=en#glossary?term=sc_qam"
    )
    ...
    assert index_redirect.headers["Location"] == "/mire/setup"
...
def test_browser_url_bootstrap_is_minimal_canonical_and_early(
    ...
    for html in (dashboard_html, settings_html, login_html, setup_html):
        ...
        assert bootstrap == {"basePath": "/mire"}
        assert set(bootstrap) == {"basePath"}
        _assert_url_helper_precedes_application_scripts(html, "/mire")
