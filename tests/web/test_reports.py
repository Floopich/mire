...
class TestReportHelpers:
    ...
    def test_ofdm_legacy_metric_health_fallback_uses_ofdm_thresholds(self):
        ...
        # Profil VOO ofdm : puissance bonne a +/-8 dBmV, MER bon des 27 dB.
        analysis = _add_builtin_analysis_meta(_analyze_downstream_channel(
            docsis="3.1",
            power=-7.0,
            quality=33.0,
    ...
    def test_sc_qam_diagnostic_notes_use_voo_critical_bounds(self):
        """Un canal hors des bornes critiques VOO produit les deux notes.

        Le courrier ne signale que les depassements de la bande critique, pas
        les simples sorties de la plage bonne. Bornes du profil VOO 256QAM :
        puissance critique +/-15 dBmV, SNR critique 30 dB. Valeurs empiriques
        (forum.voo.be), non normatives.
        """
        from app.modules.reports.report import _build_diagnostic_notes
        ...
        assert [note["type"] for note in notes] == ["ds_power_low", "snr_low"]
        assert notes[0]["spec_min"] == -15.0
        assert notes[1]["spec_min"] == 30.0

    def test_sc_qam_inside_voo_critical_bounds_produces_no_power_note(self):
        """Une puissance hors plage bonne mais dans la bande critique ne s'invite pas dans le courrier."""
        from app.modules.reports.report import _build_diagnostic_notes
    ...
    def test_legacy_historical_minimum_keeps_256qam_fallback(self):
        ...
        assert _default_warn_thresholds(worst["ds_snr_warn_min"])["snr"] == ">= 33.0 dB"
