...
class TestModulesInstall:
    ...
    def test_rejects_duplicate_builtin(self, client):
        with patch("app.blueprints.modules_bp.get_module_loader") as mock_loader:
            mock_mod = MagicMock()
            mock_mod.id = "mire.speedtest"
            mock_loader.return_value.get_modules.return_value = [mock_mod]
            resp = client.post("/api/modules/install",
                               data=json.dumps({"id": "mire.speedtest", "download_url": "https://api.github.com/test"}),
                               content_type="application/json")
            data = json.loads(resp.data)
            assert data["success"] is False
...
class TestModulesUninstall:
    ...
    def test_rejects_builtin_uninstall(self, client):
        with patch("app.blueprints.modules_bp._scan_installed_community_ids") as mock_scan, \
            ...
            mock_scan.return_value = {"mire.speedtest": "mire_speedtest"}
            mock_mod = MagicMock()
            mock_mod.id = "mire.speedtest"
            mock_mod.builtin = True
            mock_loader.return_value.get_modules.return_value = [mock_mod]
            resp = client.post("/api/modules/uninstall",
                               data=json.dumps({"id": "mire.speedtest"}),
                               content_type="application/json")
            assert resp.status_code == 403


class TestRegistryFiltering:
    """Le catalogue amont vise le marche allemand : Mire en filtre le contenu."""
    ...
    def test_german_threshold_profile_is_always_excluded(self):
        from app.blueprints.modules_bp import _filter_registry_modules

        class Cfg:
            def get(self, key, default=None):
                return True  # meme reserve actif

        ids = [m["id"] for m in _filter_registry_modules(self._mods(), Cfg())]
        assert "community.thresholds_vfkd_community" not in ids

    def test_reserved_module_is_hidden_by_default(self):
        from app.blueprints.modules_bp import _filter_registry_modules
        ...
        ids = [m["id"] for m in _filter_registry_modules(self._mods(), Cfg())]
        assert ids == ["community.other"]

    def test_reserved_module_appears_when_opted_in(self):
        from app.blueprints.modules_bp import _filter_registry_modules

        class Cfg:
            def get(self, key, default=None):
                return key == "show_reserved_modules"

        ids = [m["id"] for m in _filter_registry_modules(self._mods(), Cfg())]
        assert "community.udm_wan_monitor" in ids
