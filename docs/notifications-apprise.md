...
Mire can send alerts to an optional [Apprise API](https://github.com/caronc/apprise-api) sidecar. Mire still decides *when* to notify: severity filtering, per-event toggles, cooldowns, test notifications, and event context all stay in Mire. Apprise decides *where* to deliver the alert, for example Telegram, Matrix, ntfy, Gotify, Slack, Pushover, email, or another Apprise-supported target.
...
If the config key is empty, Mire posts to Apprise's stateless `/notify` endpoint. That mode only works when your Apprise API server has stateless target URLs configured, for example through Apprise's `APPRISE_STATELESS_URLS` environment/config option. With a config key, Mire posts to `/notify/{config_key}` and Apprise uses the persistent configuration stored under that key.

## Target examples

Provider credentials and provider-specific routing belong in Apprise, not in Mire where possible. Examples of Apprise target URLs you can manage in Apprise:

- Telegram: `tgram://BOT_TOKEN/CHAT_ID`
- Matrix: `matrixs://USER:PASSWORD@matrix.example.org/!room:example.org`
- ntfy: `ntfy://ntfy.sh/mire-alerts`
...
- Mire sends only the notification payload it already builds for alert delivery: source, timestamp, severity, event type, message, and event details.
- Mire applies severity filters, per-event toggles, and cooldowns before Apprise delivery.
...
- Apprise URL, config key, API token, and tags are treated as sensitive or secret-adjacent in Mire settings. Secret fields are masked in the UI and encrypted at rest where applicable.
...
Apprise delivery is only an alert target. It does not replace Mire's MQTT Auto-Discovery/Home Assistant telemetry integration. Keep using MQTT Auto-Discovery for sensors and Home Assistant dashboards.
