...
Mire can run at an origin root or below a same-origin path such as
`https://monitor.example.test/mire/`. The reverse proxy owns the public
boundary: it accepts only the configured mount, removes that prefix from the
path sent upstream, and forwards the remainder to Mire at `/`.
...
- `BASE_PATH` or `REVERSE_PROXY_PREFIX=N` tells Mire about its external URL
...
The proxy strips `/mire` and removes `X-Forwarded-Prefix` before forwarding.
Mire still receives `/health`, `/api/config`, and other paths at the origin
...
The trailing slash on `proxy_pass` makes Nginx strip the matched `/mire/`
prefix. Configure the container with `BASE_PATH=/mire`.
...
origin to forward `/health`, `/api`, `/static`, `/sw.js`, or any other Mire
...
Mire Core does not query a platform supervisor or consume platform identity
headers. A wrapper may obtain its platform-assigned mount and set an explicit
`BASE_PATH` before Mire starts. The wrapper must still enforce prefix
stripping and the same-origin boundary described above.

Home Assistant can authenticate access to its Ingress gateway, but Mire
Core does not inherit Home Assistant identity or session state. Mire's own
admin-password boundary remains active; apply the same network-access controls
you would use for any other Mire deployment.
