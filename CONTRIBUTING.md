...
Mire v2.0 uses a **modular collector-based architecture**. All data collection follows this pattern:
...
```bash
git clone https://github.com/floopich/mire.git
cd mire
...
Mire is used internationally and translations from native speakers are welcome. To add a new core language:
...
Mire supports community modules that extend functionality without modifying core code. Modules can add API endpoints, data collectors, settings panels, dashboard tabs, and more.
...
Mire may be mounted below a reverse-proxy path such as `/mire`. Browser code in core and community modules must pass every app-owned root-relative URL through `window.mireUrl(path)` at the point where it is used. This includes `fetch()` targets, constructed API URLs, generated `href`/`src` attributes, downloads, and internal navigation:

```javascript
fetch(mireUrl('/api/example'));
image.src = mireUrl('/api/example/image/' + id);
```

The helper is loaded before module scripts on Mire's standalone pages. It accepts only a safe internal string beginning with exactly one `/`, preserves query strings and fragments, and throws for relative, absolute, protocol-relative, traversal, malformed, or ambiguously encoded input. Modules should not catch that error and retry with an unprefixed URL.
